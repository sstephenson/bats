...
# Fail and display an error message if `$status' is not zero. The error
...
#   1 - otherwise
# Outputs:
#   STDERR - `$status' and `$output', if status check fails
...
assert_success() {
  ...
  if (( status != 0 )); then
    ...
    batslib_print_kv_single_or_multi 8 \
        'expected' "$expected" \
        'actual'   "$output" \
      | batslib_decorate 'command succeeded, but output differs' \
      | flunk
  fi
}
